...
def _mock_decision(
    tier="medium", model="gemini-2.5-flash", task_type="reasoning", complexity="standard", confidence=0.85
):
    d = MagicMock()
    d.tier.value = tier
...
def test_get_chat_model_google():
    ...
    with patch("classifier.classify_task") as mock_classify:
        ...
        langchain_google_genai.ChatGoogleGenerativeAI.assert_called_once_with(model="gemini-2.5-flash")
...
def test_get_chat_model_uses_fallback_on_error():
    ...
    with patch("classifier.classify_task", side_effect=ClassificationError("boom")):
        ...
        langchain_google_genai.ChatGoogleGenerativeAI.assert_called_with(model="gemini-2.5-flash")
