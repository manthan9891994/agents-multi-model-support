...
def test_router_uses_custom_provider():
    register_provider(
        "cohere_test",
        {
            ModelTier.LOW: "command-r",
            ModelTier.MEDIUM: "command-r-plus",
            ModelTier.HIGH: "command-r-plus",
        },
    )
...
def test_layer2_provider_dispatch():
    ...
    fake_caller = MagicMock(
        return_value=MagicMock(
            text='{"task_type":"reasoning","complexity":"simple","confidence":0.9,"reason":"x"}'
        )
    )
...
def test_post_classify_hook_modifies_decision():
    ...
    router = Router(
        post_classify_hooks=[force_reasoning], layer2_enabled=False, layer3_enabled=False, cache_enabled=False
    )
...
def test_hook_context_passed_through():
    ...
    router = Router(
        post_classify_hooks=[capture_hook], layer2_enabled=False, layer3_enabled=False, cache_enabled=False
    )
    router.classify(
        "Hi some unique tenant context xyz123", hook_context={"tenant_id": "acme", "user_id": "u123"}
    )
