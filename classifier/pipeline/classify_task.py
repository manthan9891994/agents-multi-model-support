"""The cascade pipeline — classify_task and its supporting state.

This used to live inline in ``classifier/__init__.py``. It is the heart of
the package: every public entry point eventually calls ``classify_task``.

Flow:

    classify_task()
        ├─ pre-classify hooks
        ├─ custom_classifier escape hatch
        ├─ streaming-debounce check
        ├─ input validation (provider, length, non-empty)
        ├─ budget guard (downgrade or refuse)
        ├─ exact + semantic cache lookup
        ├─ single-flight coalescing → _classify_inner
        │       └─ _classify_inner_traced  (the L1→L3→L2 cascade itself
        │            + multimodal / tools / context-window / PII /
        │              capability filtering / latency SLA / residency
        │              tier adjustments)
        └─ post-classify hooks → return ClassificationDecision
"""
...
MAX_TASK_CHARS = int(os.environ.get("DMR_MAX_TASK_CHARS", "32000"))

# Streaming debounce — last known good decision (stateless fallback).
...
# Calibration data (loaded once at first use)
...
def classify_task(
    ...
    Args:
        ...
        task_stable:     Streaming debounce — set False while user is still typing.
        user_id:         Per-user tier personalization key.
    ...
    # Streaming debounce — return last decision while input is in-flight
    if not task_stable:
    ...
    # ── Single-flight coalescing — compute once per unique task ──────────────
...
def _classify_inner_traced(
    ...
    # ── Layer 2 (check L2 budget before firing) ───────────────────────────────
    l2_result = None
    l2_fired = (
    ...
    # ── L1 + L2 agreement boost / disagreement flag ──────────────────────────
    disagreement = False
    ...
    # ── Tool-aware routing — bump tier for first planning call ────────────────
    if (
    ...
    # ── PII detection → policy-driven tier bump and compliance_flag ───────────
...
# Lazy import: hook_manager is referenced inside classify_task. The import
# happens inside the function so that hooks.py can import classify_task back
# (e.g. for type hints) without a circular load.
