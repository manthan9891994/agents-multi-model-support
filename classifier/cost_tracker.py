...
# Approximate blended cost per 1M tokens (input + output averaged)
...
class CostTracker:
    ...
    def record(self, model: str, input_tokens: int, output_tokens: int = 500) -> float:
        """Record a model call and return the estimated cost."""
    ...
    def should_downgrade(self) -> bool:
        """Returns True when >80% of the monthly budget is consumed.
        Callers should cap tier at MEDIUM when this is True."""
        over_limit = self.budget_utilization > 0.80
        if over_limit:
            logger.warning(
    ...
    def is_exhausted(self) -> bool:
        """Returns True when budget is fully consumed. All calls go to LOW tier."""
        exhausted = self.budget_utilization >= 1.0
        if exhausted:
            logger.error("Monthly budget exhausted — forcing LOW tier for all requests.")
        return exhausted
